...
pub fn type_check(
    ...
    for module_sym in module_order {
        ...
        for &value_sym in value_order {
            let global_id: GlobalId<node::ValueBind> =
                bindings.lookup_value_bind_by_sym(value_sym).unwrap();

            let typer = Typer::new(global_id, spans.clone(), &type_env, &bindings);
            ...
            type_env.insert_value(value_sym, typed_nodes.meta(global_id.id).clone());
