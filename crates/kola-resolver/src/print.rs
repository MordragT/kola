...
impl<'a> Decorator<'a> for ResolutionDecorator<'a> {
    fn decorate(&self, notation: Notation<'a>, with: AnyId, arena: &'a Bump) -> Notation<'a> {
        let sym = match with {
            AnyId::BindPat(id) => self.0.bind_pats.get_unchecked(id).red().display_in(arena),
            ...
            AnyId::RecordFieldPat(id) => self
                ...
                .red()
                .display_in(arena),
            AnyId::LetExpr(id) => self.0.let_exprs.get_unchecked(id).red().display_in(arena),
            ...
            AnyId::QualifiedType(id) => self
                ...
                .red()
                .display_in(arena),
            AnyId::TypeVar(id) => self.0.type_vars.get_unchecked(id).red().display_in(arena),
            AnyId::TypeVarBind(id) => self
                ...
                .red()
                .display_in(arena),
            AnyId::ValueBind(id) => self.0.value_binds.get_unchecked(id).red().display_in(arena),
            AnyId::TypeBind(id) => self.0.type_binds.get_unchecked(id).red().display_in(arena),
            AnyId::ModuleBind(id) => self
                .0
                .module_binds
                .get_unchecked(id)
                .red()
                .display_in(arena),
            AnyId::ModuleTypeBind(id) => self
                .0
                .module_type_binds
                .get_unchecked(id)
                .red()
                .display_in(arena),
            AnyId::FunctorBind(id) => self
                .0
                .functor_binds
                .get_unchecked(id)
                .red()
                .display_in(arena),
            AnyId::ModuleBody(id) => self
                .0
                .module_bodies
                .get_unchecked(id)
                .red()
                .display_in(arena),
            AnyId::ModulePath(id) => self
                .0
                .module_paths
                .get_unchecked(id)
                .red()
                .display_in(arena),
            AnyId::ModuleImport(id) => self
                .0
                .module_imports
                .get_unchecked(id)
                .red()
