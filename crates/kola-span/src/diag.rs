...
impl Report {
    ...
    pub fn split_reset(&mut self, checkpoint: ReportCheckpoint) -> Self {
        ...
        let diagnostics = self.diagnostics.split_off(checkpoint.diagnostics);
        Self {
            issues,
            diagnostics,
        }
    ...
    /// The first diagnostic in the report will be used as the main diagnostic,
    /// and all other issues and diagnostics will be added as notes and trace elements to it.
    pub fn flatten(mut self) -> Option<Diagnostic> {
        let mut diag = self.diagnostics.pop()?;

        for issue in self.issues.into_iter().rev() {
            diag = diag.with_note(issue.to_string());
        }

        diag = diag.with_trace(
            self.diagnostics
                .into_iter()
                .rev()
                .map(|d| (d.to_string(), d.loc)),
        );
...
impl Diagnostic {
    ...
    pub fn with_trace_element(mut self, loc: Loc, label: impl Into<String>) -> Self {
    ...
    /// * `label` - The trace label
    /// * `loc` - The source location associated with the trace
    pub fn add_trace_element(&mut self, loc: Loc, label: impl Into<String>) -> &mut Self {
        self.trace.push((label.into(), loc));
        self
    }
