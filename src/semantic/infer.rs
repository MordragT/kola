use crate::{source::Source, syntax::visit::Visitable};

use super::{error::SemanticReport, Scopes, Substitution};

pub trait Inferable: Visitable {
    fn infer(&mut self, source: &Source) -> Result<()> {}
}
...
// use super::{
//     error::SemanticReport,
//     types::{MonoType, PolyType, TypeVar},
//     Constraints, Scopes, Substitutable,
// };
...
//     pub fn infer<N>(&mut self, node: &mut N) -> Result<(), SemanticReport>
//     where
//         N: Visitable,
//     {
...
//     fn type_of<'a>(&self, expr: &'a ast::Expr) -> ControlFlow<SemanticReport, &'a MonoType> {
//         match expr.ty() {
//             Err(e) => ControlFlow::Break(SemanticReport::new(
//                 Vec::new(),
//                 e.span,
...
//     fn try_get(&self, ident: &ast::IdentExpr) -> ControlFlow<SemanticReport, &PolyType> {
...
//     type BreakValue = SemanticReport;
