...
use super::{
    ...
    tree::{self, Node, NodeId, SyntaxTree},
    Span,
};
    ...
    I: ValueInput<'src, Token = Token<'src>, Span = Span>,
{
        ...
        T: Into<Node>,
    {
        self.map_with(|node, e| {
            let span = e.span();
            let tree: &mut SimpleState<SyntaxTree> = e.state();
            tree.insert(node, span)
        })
    }

    #[inline]
    fn to_expr(self) -> impl Parser<'src, I, tree::Expr, Extra<'src>>
    where
        T: Into<tree::Expr>,
    {
        self.map(Into::into)
...
pub type Extra<'src> = extra::Full<Rich<'src, Token<'src>, Span>, SimpleState<SyntaxTree>, ()>;
    ...
    recursive(|expr| {
        ...
        let record_op = ident
            ...
            .foldl_with(inner_op, |source, op, e| {
                ...
                match op {
                    RecordOp::Extend(field, value) => tree.insert(
                        tree::Expr::from(tree::RecordExtend {
                            source,
                            field,
                            value,
                        }),
                    ...
                    RecordOp::Restrict(field) => tree.insert(
                        tree::Expr::from(tree::RecordRestrict { source, field }),
                        span,
                    ),
                    RecordOp::Update(field, value) => tree.insert(
                        tree::Expr::from(tree::RecordUpdate {
                            source,
                            field,
                            value,
                        }),
        ...
        let select = atom
            .foldl_with(
                ...
                |source, field, e| {
                    let span = e.span();
                    let tree: &mut SimpleState<SyntaxTree> = e.state();
                    tree.insert(tree::Expr::from(tree::RecordSelect { source, field }), span)
                },
            )
            .boxed();
        ...
        let unary = unary_op
            ...
            .foldr_with(select, |op, target, e| {
                let span = e.span();
                let tree: &mut SimpleState<SyntaxTree> = e.state();
                tree.insert(tree::Expr::from(tree::Unary { op, target }), span)
            })
            .boxed();
        ...
        let product = unary
            ...
            .foldl_with(op.then(unary).repeated(), |left, (op, right), e| {
                let span = e.span();
                let tree: &mut SimpleState<SyntaxTree> = e.state();
                tree.insert(tree::Expr::from(tree::Binary { op, left, right }), span)
            })
            .boxed();
